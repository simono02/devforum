  ...
  <header class="topbar">
    <div class="topbar-left">
      ...
      <div class="breadcrumb">
        <router-link to="/" class="brand">dev<span class="accent">forum</span></router-link>
    ...
    <div class="topbar-right">
      <!-- Theme toggle -->
      <button class="icon-btn" @click="themeStore.toggle()" :title="isDark ? 'Light mode' : 'Dark mode'">
      ...
      <!-- Write post -->
      <router-link to="/dashboard/posts" class="write-btn" v-if="!isMobile">
      ...
      <div class="user-menu" ref="menuRef">
        <button class="user-trigger" @click="menuOpen = !menuOpen">
          ...
          <svg class="chevron" :class="{ open: menuOpen }" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><polyline points="6 9 12 15 18 9"/></svg>
        </button>
        ...
        <transition name="dropdown">
          <div class="dropdown" v-if="menuOpen">
            ...
            <router-link class="dropdown-item" to="/dashboard" @click="menuOpen = false">
            ...
            <router-link class="dropdown-item" to="/dashboard/profile" @click="menuOpen = false">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
              Profile
            </router-link>
            <router-link class="dropdown-item" to="/dashboard/posts" @click="menuOpen = false">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
              My Posts
            </router-link>            <div class="dropdown-divider"></div>
...
const pageTitle = computed(() => titles[route.path] || 'DevForum')
...
.menu-btn {
  background: none; border: none; cursor: pointer; color: var(--text-ghost);
  padding: 0.4rem; border-radius: 6px; display: flex; align-items: center;
...
.brand { font-size: 1.1rem; font-weight: 800; color: var(--text-primary); text-decoration: none; }
.accent { color: var(--accent); }
.bc-sep { width: 14px; height: 14px; color: var(--text-ghost); }
.page-title { font-size: 0.9rem; font-weight: 700; color: var(--text-secondary); }

.icon-btn {
  background: none; border: none; cursor: pointer; color: var(--text-ghost);
  padding: 0.45rem; border-radius: 8px; display: flex; align-items: center;
  transition: color 0.2s, background 0.2s;
}
...
.write-btn svg { width: 13px; height: 13px; }
...
.dropdown-header { display: flex; align-items: center; gap: 0.75rem; padding: 0.875rem 1rem; }
.dh-av {
